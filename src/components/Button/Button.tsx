...
const Button: React.FC<ButtonProps> = ({ onClick, type, htmlType = "button", disabled, children }) => {
  ...
  return (
    <button
      className={`text-xl h-fit font-bold font-roboto disabled:bg-buttonBackground/50 disabled:text-buttonText/25 py-3 px-6 min-w-full sm:min-w-36 transition-colors ${typeStyles}`}
      onClick={onClick}
